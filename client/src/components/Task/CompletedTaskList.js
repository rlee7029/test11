...
import { GET_COMPLETED_TASKS_BY_USER_ID } from '../../graphql/queries';
...
const TaskList = () => {
  ...
  const { loading, error, data } = useQuery(GET_COMPLETED_TASKS_BY_USER_ID, {
    variables: { userId },
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      await deleteTask({
        variables: { id: taskId },
        refetchQueries: [{ query: GET_COMPLETED_TASKS_BY_USER_ID, variables: { userId } }],
      });
    } catch (error) {
      console.log(error);
    }
